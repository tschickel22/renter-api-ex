...
import {getDeclarations} from "../../../slices/insuranceSlice";
...
const InsuranceDeclarationsView = ({insurance, readOnly, declarationsBatchNumber}) => {
    ...
    useEffect(async() => {
        if (insurance.hash_id) {
            // Load existing attachments
            const results = await store.dispatch(getDeclarations({insuranceId: insurance.hash_id})).unwrap()
    ...
    async function handleUpload(currentFile) {
        ...
        return client.post("/api/internal/insurances/" + (insurance.hash_id || "new") + "/upload_declarations", formData, {forUpload: true})
            .then((response) => {
    ...
    async function handleDelete(e, declaration) {
        ...
        // Request made to the backend api
        // Send formData object
        return client.post("/api/internal/insurances/" + (insurance.hash_id || "new") + "/destroy_declaration", formData)
            .then((response) => {
                    if (response.success) {
...
export default InsuranceDeclarationsView;
