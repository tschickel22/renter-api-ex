...
for i in 1..number_of_workers do
    God.watch do |w|
      queues          = ['announcements_find', 'announcements_populate', 'announcements_deliver']
      queues_str      = queues.join(',')

      w.name          = "resque-announcements-#{i}"
      ...
      w.log           = File.expand_path(File.join(File.dirname(__FILE__), '..','log', "announcements-#{i}.log"))
